@php
    ...
    $canManageForm = isset($canManage) ? $canManage : (!$isEmpRole && (isSuperAdmin() || \App\Services\AuthorizationEngine::isHRRole($authUser) || can('manage_funeral_supports')));
@endphp
...
<div class="col-md-3">
    <div class="form-group mb-3">
        ...
        {!! Form::date('support_date', isset($funeralSupport) ? $funeralSupport->support_date : date('Y-m-d'), ['class' => 'form-control', 'required' => 'required']) !!}
    </div>
</div>
...
<div class="col-md-3">
    <div class="form-group mb-3">
        {!! Form::label('attachment', 'Document / Certificate Attachment:', ['class' => 'fw-semibold mb-1']) !!}
        <input type="file" name="attachment" class="form-control" accept=".pdf,.png,.jpg,.jpeg,.doc,.docx">
        @if(isset($funeralSupport) && $funeralSupport->attachment)
            <small class="d-block mt-1"><a href="{{ asset($funeralSupport->attachment) }}" target="_blank" class="text-primary"><i class="im im-icon-File-TXT me-1"></i>View Current Document</a></small>
        @endif
    </div>
</div>
...
<div class="col-md-12">
    <div class="form-group mb-3">
        ...
        {!! Form::textarea('remarks', null, ['class' => 'form-control', 'rows' => 3, 'placeholder' => 'Enter funeral support details...']) !!}
    </div>
</div>
...
<div class="col-12 mt-3">
    ...
    <a href="{{ route('funeralSupports.index') }}" class="btn btn-outline-secondary rounded-pill px-4 ms-2">Cancel</a>
</div>
