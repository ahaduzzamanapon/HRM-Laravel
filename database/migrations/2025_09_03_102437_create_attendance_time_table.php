<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('attendance_time', function (Blueprint $table) {
            $table->increments('time_attendance_id');
            $table->integer('employee_id');
            $table->integer('office_shift_id')->nullable();
            $table->date('attendance_date');
            $table->string('clock_in')->nullable();
            $table->string('clock_out')->nullable();
            $table->string('lunch_in', 32)->nullable();
            $table->string('lunch_out', 32)->nullable();
            $table->tinyInteger('late_status')->nullable();
            $table->smallInteger('production')->default(0);
            $table->smallInteger('ot')->default(0);
            $table->smallInteger('late_time')->default(0);
            $table->tinyInteger('move_status')->default(0)->comment('1=move_leave, 0=no_leave');
            $table->tinyInteger('lunch_late_status')->default(0);
            $table->tinyInteger('early_out_status')->default(0);
            $table->string('clock_in_ip_address')->nullable();
            $table->string('clock_out_ip_address')->nullable();
            $table->string('clock_in_out')->nullable();
            $table->text('comment')->nullable();
            $table->string('clock_in_latitude', 150)->nullable();
            $table->string('clock_in_longitude', 150)->nullable();
            $table->string('clock_out_latitude', 150)->nullable();
            $table->string('clock_out_longitude', 150)->nullable();
            $table->string('time_late')->nullable();
            $table->string('early_leaving')->nullable();
            $table->string('overtime')->nullable();
            $table->string('total_work')->nullable();
            $table->string('total_rest')->nullable();
            $table->string('attendance_status', 100)->nullable();
            $table->string('status', 24)->nullable();
            $table->tinyInteger('extra_ap')->default(0);
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('attendance_time');
    }
};